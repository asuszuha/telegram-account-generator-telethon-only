...
class AddTgUsersMt(AbstractTab):
    ...
    def _create_first_column(self):
        ...
        rd_btn_scrape.grid(row=0, column=0, sticky="w", padx=(10, 5))
        rd_btn_add_user.grid(row=1, column=0, sticky="w", padx=(10, 5))
        ...
        lbl_frame_add_user.grid(row=1, column=0, sticky="we", padx=(5, 5))
        rd_btn_session_file.grid(row=0, column=0, sticky="w", padx=(10, 5))
        rd_btn_phones_file.grid(row=1, column=0, sticky="w", padx=(10, 5))

        # Threading UI
        self.var_multi_thread = IntVar(value=0)
        lbl_frame_mt = ttk.LabelFrame(self, text="Select Threading Option:")
        rd_btn_single_thread = ttk.Radiobutton(
            lbl_frame_mt, text="0: Single Thread", value=0, variable=self.var_multi_thread
        )
        rd_btn_multi_thread = ttk.Radiobutton(
            lbl_frame_mt, text="1: Multi Thread", value=1, variable=self.var_multi_thread
        )

        lbl_frame_mt.grid(row=2, column=0, sticky="we", padx=(5, 5))
        rd_btn_single_thread.grid(row=0, column=0, sticky="w", padx=(10, 5))
        rd_btn_multi_thread.grid(row=1, column=0, sticky="w", padx=(10, 5))
        ...
        rd_btn_code_required_no.grid(row=0, column=1, sticky="w", padx=(10, 5))
        rd_btn_code_required_yes.grid(row=1, column=1, sticky="w", padx=(10, 5))
        ...
        lbl_delay_for_user.grid(row=1, column=1, sticky="we", padx=(5, 5))
        lbl_user_delay.grid(row=0, column=1, sticky="w", padx=(5, 5))
        self._spb_delay_for_adding.grid(row=1, column=1, sticky="w", padx=(10, 5))
        ...
        _chb_proxy_enabled.grid(row=2, column=1, sticky="w", padx=(5, 5))

        # Scraping options
        self.var_recent_users = IntVar(value=0)
        lbl_frame_recent_user = ttk.LabelFrame(self, text="Scraping options:")
        rd_btn_scrape_all_users = ttk.Radiobutton(
            lbl_frame_recent_user, text="0: Return all possible users", value=0, variable=self.var_recent_users
        )
        rd_btn_scrape_recent_users = ttk.Radiobutton(
            lbl_frame_recent_user,
            text="1: Return with start date \n and end date",
            value=1,
            variable=self.var_recent_users,
        )
        ...
        # )
        lbl_cal_start = ttk.Label(lbl_frame_recent_user, text="Start Date: ")
        self.cal_start = DateEntry(lbl_frame_recent_user, selectmode="day")

        lbl_frame_recent_user.grid(row=1, column=2, sticky="we", padx=(5, 5))
        rd_btn_scrape_all_users.grid(row=0, column=2, sticky="w", padx=(10, 5))
        rd_btn_scrape_recent_users.grid(row=1, column=2, sticky="w", padx=(10, 5))
        # rd_btn_scrape_with_date.grid(row=2, column=2, sticky="w", padx=(10, 5))
        lbl_cal_start.grid(row=2, column=2, sticky="w", padx=(10, 5))
        self.cal_start.grid(row=2, column=2, sticky="e", padx=(10, 5))

        lbl_cal_end = ttk.Label(lbl_frame_recent_user, text="End Date: ")
        self.cal_end = DateEntry(lbl_frame_recent_user, selectmode="day")

        # rd_btn_scrape_with_date.grid(row=2, column=2, sticky="w", padx=(10, 5))
        lbl_cal_end.grid(row=3, column=2, sticky="w", padx=(10, 5))
        self.cal_end.grid(row=3, column=2, sticky="e", padx=(10, 5))

        # User id only
        self.var_user_id_only = IntVar(value=0)
        lbl_frame_user_id = ttk.LabelFrame(self, text="Adding & Scraping options:")
        rd_btn_add_only_username = ttk.Radiobutton(
            lbl_frame_user_id, text="0: Only with username", value=0, variable=self.var_user_id_only
        )
        rd_btn_add_all_users = ttk.Radiobutton(
            lbl_frame_user_id, text="1: Only with User ID", value=1, variable=self.var_user_id_only
        )
        lbl_frame_user_id.grid(row=0, column=2, sticky="we", padx=(5, 5))
        rd_btn_add_only_username.grid(row=0, column=2, sticky="w", padx=(10, 5))
        rd_btn_add_all_users.grid(row=1, column=2, sticky="w", padx=(10, 5))
    ...
    def run(self):
        ...
        if not self.frame_thread or not self.frame_thread.is_alive():
            self.frame_thread = AddUserMt(
                ...
                user_add_option=self.var_user_id_only.get(),
